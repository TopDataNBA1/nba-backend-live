...
def detectar_adelantamientos(anterior, actual):
    ...
    for idx, jugador in enumerate(actual):
        ...
        if adelantados:
            lista_adelantados = ", ".join(reversed(adelantados))  # 👈 Reverse order
            print(f"🔔 {nombre} ({puntos_hoy} points today) has passed {lista_adelantados} "
                  f"and is now ranked {ordinal(posicion_actual)} in playoffs all-time with {puntos_actuales} total points.")
